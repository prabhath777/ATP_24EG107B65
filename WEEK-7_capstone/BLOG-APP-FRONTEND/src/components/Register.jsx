  ...
  const onUserRegister = async (userObj) => {
   ...
   console.log(profileImageUrl)
    try {
      //start loading
      setLoading(true);
      //make HTTP POST req to create User in backend (axiosInstance has baseURL)
      let res = await axios.post("/auth/user", formData);
      ...
      console.log("err in registration", err.response?.data || err.message || err);
      setApiError(
        err.response?.data?.message || err.response?.data?.error || err.message || "Registration failed"
      );
    } finally {
      setLoading(false);
    }
